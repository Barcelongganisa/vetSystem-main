...
class User {
    ...
    public function create($name, $email, $cNumber,$password) {
        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
        $activation_hash = bin2hex(random_bytes(16)); // More secure activation hash

        $sql = "INSERT INTO user (name, role, email, number, password_hash, account_activation_hash) VALUES (?, 'user', ?, ?, ?, ?)";
        $stmt = $this->mysqli->prepare($sql);
        $stmt->bind_param("sssss", $name, $email, $cNumber, $hashed_password, $activation_hash);
